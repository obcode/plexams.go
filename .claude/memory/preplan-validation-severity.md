...
**Two distinct "pre-plannings" — do not confuse them (wiring correction 2026-07-05):**
1. **EXaHM/SEB Vorplanung (the solver)**: data = `preplan_exams` (synthetic `PreplanExam` pseudo-exams, own IDs, ExpectedStudents, PlannedDay/Slot) + Anny capacities/bookings; generated by `generatePreplanAssignment`; validated by **`validatePreplanAssignment`** (Query → the graded `PreplanValidation` above). THIS is "die Vorplanung".
2. **Manual room pre-assignment**: data = `rooms_pre_planned` (real exam ancode + concrete room), written only by the manual `prePlanRoom` mutation; validated by **`validatePrePlannedExahmRooms`** (Subscription/LogLine). Reads `assembled_exams` + `rooms_pre_planned` + `plan`; NEVER touches `preplan_exams`.

The GUI "EXaHM Vorplanung Räume" validation was wired to `validatePrePlannedExahmRooms` (pipeline 2), which does not validate the solver's output at all — the source of the "the validator doesn't match the pre-planning" mismatch. Decision: **GUI should call `validatePreplanAssignment`** for the Vorplanung; `validatePrePlannedExahmRooms` stays in the backend for its real phase-2 purpose (manual room pre-assignment). No backend change needed (validatePreplanAssignment already graded). See [[gui-and-cli-sync]].
