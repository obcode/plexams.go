...
//
// goose owns the *structure*. It is deliberately NOT merged with the per-semester
// semester.schema_version (see PG.Migrate), which versions the *shape of the data*
// inside a semester -- two different questions with two different lifetimes.
//
...
//
// This is fatal on failure: a half-migrated schema means the queries compiled into
// this binary do not match the database, and every later error would be a
// confusing symptom of that one cause.
