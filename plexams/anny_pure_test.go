...
func TestNormRoomName(t *testing.T) {
	// normRoomName (preplan_booking.go) and normalizeRoomName (anny.go) must stay identical;
	// several call sites inline the same expression. Pin both.
...
func TestIsApprovedAnnyStatus(t *testing.T) {
	tests := []struct {
		status string
		want   bool
	}{
	...
	for _, tt := range tests {
		if got := isApprovedAnnyStatus(tt.status); got != tt.want {
			t.Errorf("isApprovedAnnyStatus(%q) = %v, want %v", tt.status, got, tt.want)
		}
	}
}
