...
var fixedStart = time.Date(2026, 1, 12, 15, 4, 0, 0, time.UTC)

func startPtr() *time.Time { s := fixedStart; return &s }
...
func TestProgramRows(t *testing.T) {
	exams := []*model.PlannedExam{
		...
		plannedExam("M2", "Zimmer", &model.PlanEntry{Starttime: startPtr()},
			[]*model.EnhancedPrimussExam{section("IF", 502, 3)}, nil),
		// ancode 501, two rooms (one NTA, one reserve) -> two rows, sorts before 502
		plannedExam("M1", "Albers", &model.PlanEntry{Starttime: startPtr()},
...
func TestExahmRows(t *testing.T) {
	...
	exams := []*model.PlannedExam{
		// EXaHM, one room, planned
		{
			...
			PlanEntry:    &model.PlanEntry{Starttime: startPtr()},
