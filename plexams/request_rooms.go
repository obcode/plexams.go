...
func (p *Plexams) RequestRooms() error {
	...
	for _, slot := range p.semesterConfig.Slots {
		...
		for _, exam := range examsInSlot {
			...
			reqs := 0
			for _, req := range exam.Exam.StudentRegs {
				reqs += len(req.StudentRegs)
			}
	...
	for _, slot := range p.semesterConfig.Slots {
		if needBigRooms[slot.DayNumber][slot.SlotNumber] > 0 {
			fmt.Printf("      - [%d,%d] # %s, %s\n", slot.DayNumber, slot.SlotNumber,
				p.semesterConfig.Days[slot.DayNumber-1].Date.Format("02.01.06"), slot.Starttime.Format("15:04"))
		}
	}
	fmt.Println("  R1.049:\n    allowedSlots:")
	for _, slot := range p.semesterConfig.Slots {
	...
	for _, day := range p.semesterConfig.Days {
		...
		for i, slot := range p.semesterConfig.Starttimes {
			if needBigRooms[day.Number][i+1] > 0 {
				...
				fmt.Printf("    - %v - %v\n",
					starttime.Add(-30*time.Minute).Format("15:04"),
					starttime.Add(120*time.Minute).Format("15:04"))
			}
		}
	}
	fmt.Println("- R1.049")
